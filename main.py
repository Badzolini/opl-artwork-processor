...
def rename_file(filename, suffix):
    ...
    name, ext = os.path.splitext(filename)  # Extract filename without extension
...
def convert_image(filename, suffix, input_folder, output_folder):
    """Converts a single image from JPG, JPEG, or PNG to PNG format with renamed output."""
    global graceful_shutdown
    ...
    if filename.lower().endswith((".jpg", ".jpeg", ".png")):
        ...
        try:
            ...
            log_message(f"Processed {filename} -> {new_filename}")
...
def main():
    if len(sys.argv) != 4:
        log_message(
            "Usage: python main.py <path/to/artfolder/> <path/to/outputfolder/> <suffix>")
        sys.exit(1)
    ...
    with ThreadPoolExecutor(max_threads) as executor:
        future_to_filename = {executor.submit(
            convert_image, f, suffix, input_folder, output_folder): f for f in images_to_process}
        try:
